...
- Migukstory의 차별점은 시각화입니다. 초안에 유용한 시각화가 없으면 반드시 `## 한눈에 보는 변화` 섹션을 추가하고, 기사 성격에 맞게 번호 매긴 단계별 체크리스트, 짧은 타임라인, 또는 Markdown 비교표 중 하나 이상을 넣으세요. Mermaid, flowchart TD, sequenceDiagram 등 코드형 다이어그램은 사이트에서 깨질 수 있으므로 절대 넣지 마세요. 절차/자격/이민/혜택/정책 변경 글은 모바일에서 바로 읽히는 표/목록 시각화 없이는 불완전한 글로 봅니다.
